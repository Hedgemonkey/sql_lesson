...
# create a new instance of sessionmaker, then bind it to the engine (db)
Session = sessionmaker(db)

# create a new session
session = Session()

# creating the database using the declarative_base
...
# artists = session.query(Artist).filter(Artist.Name == "Queen").all()
# for artist in artists:
#     print(artist.ArtistId, artist.Name, sep=" | ")

# Query 4 - select only by "ArtistId" #51 from the "Artist" table
# artist = session.query(Artist).filter(Artist.ArtistId == 51).first()
# print(artist.ArtistId, artist.Name, sep=" | ")
